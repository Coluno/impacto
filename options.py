...
# Função para realizar o teste de estresse 
...
def plotar_grafico_cascata(df):
    ...
    fig = go.Figure(go.Waterfall(
        ...
        y=valores,
        textposition="inside",
        text=[f"{v:,.2f}" for v in valores],  # Formatação dos valores
...
# Interface do Streamlit
st.title("Simulação de Teste de Estresse")
...
min_hipotetico = st.number_input("Digite o valor mínimo hipotético do dólar (R$):", format="%.2f")
max_hipotetico = st.number_input("Digite o valor máximo hipotético do dólar (R$):", format="%.2f")
intervalo = st.number_input("Digite o intervalo entre valores hipotéticos (padrão 0.10):", min_value=0.01, value=0.10, format="%.2f")

# Botão para rodar a simulação
if st.button("Rodar Teste de Estresse"):
    if min_hipotetico >= max_hipotetico:
        st.error("O valor mínimo hipotético deve ser menor que o valor máximo.")
    else:
        # Realiza o teste de estresse
        df_resultado = teste_stresse(venda_media, valor_total, min_hipotetico, max_hipotetico, intervalo)

        # Exibe a tabela de resultados
        st.subheader("Resultados do Teste de Estresse")
        st.dataframe(df_resultado)

        # Exibe o gráfico de cascata
        st.subheader("Gráfico de Cascata")
        st.plotly_chart(plotar_grafico_cascata(df_resultado), use_container_width=True)
